...
const WorkflowDashboard = ({
  ...
  const isOrderUpdateRelevant = useCallback(
    (updateData) => {
      ...
      if (updateType === "status_change") {
        ...
        const isOldStatusRelevant =
          oldStatus && roleStatuses.includes(oldStatus);
  ...
  const getRoleNotificationMessage = useCallback(
    (updateData) => {
      const { orderId, newStatus, oldStatus, updateType } = updateData;
      const roleStatuses = getRoleStatuses();
  ...
  const addOrderNotification = useCallback(
    (updateData) => {
      ...
      addNotification({
        ...
        duration: 8002, // 8 seconds
        autoHide: true,
      });
    },
    [
      isOrderUpdateRelevant,
      getRoleNotificationMessage,
      addNotification,
      userRole,
    ]
  );
  ...
  const handleOrderUpdate = useCallback(
    (updateData) => {
      ...
      if (!isOrderUpdateRelevant(updateData)) {
        console.log(
          "🚫 Order update not relevant to current role, skipping notification"
        );
      ...
      if (updateData.updateType === "status_change") {
        ...
        setAvailableOrders((prev) => {
          ...
          console.log(
            `🗂️ Removed Order #${orderId} from Available (${prev.length} → ${filtered.length})`
          );
          return filtered;
        });
        setWorkingOrders((prev) => {
          ...
          console.log(
            `🗂️ Removed Order #${orderId} from Working (${prev.length} → ${filtered.length})`
          );
          return filtered;
        });
        setHistoryOrders((prev) => {
          ...
          console.log(
            `🗂️ Removed Order #${orderId} from History (${prev.length} → ${filtered.length})`
          );
          return filtered;
        });
        ...
        if (orderData && roleStatuses.includes(newStatus)) {
          ...
          if (userRole === "Manager" && newStatus === "pending") {
            ...
            setAvailableOrders((prev) => {
              ...
              console.log(
                `✅ Added Order #${orderId} to Available (Manager/pending) (${prev.length} → ${newOrders.length})`
              );
              return newOrders;
            });
            ...
            setWorkingOrders((prev) => {
              const newOrders = [...prev, updatedOrder];
              console.log(
                `✅ Added Order #${orderId} to Working (${prev.length} → ${newOrders.length})`
              );
              return newOrders;
            });
            ...
            setAvailableOrders((prev) => {
              const newOrders = [...prev, updatedOrder];
              console.log(
                `✅ Added Order #${orderId} to Available (${prev.length} → ${newOrders.length})`
              );
              return newOrders;
            });
            ...
            console.log(
              `➡️ Order #${orderId} assigned to someone else (${assignedWorker})`
            );
          }
        } else {
          ...
          if (orderData) {
            setHistoryOrders((prev) => {
              ...
              console.log(
                `📚 Added Order #${orderId} to History (${prev.length} → ${newOrders.length})`
              );
              return newOrders;
            });
        ...
        const isAssignedToMe =
          assignedWorkerId?.toString() === workerId?.toString();
        ...
        if (isAssignedToMe) {
          ...
          if (orderToMove) {
            ...
            setWorkingOrders((prev) => [
              ...prev,
              { ...orderToMove, assigned_worker: assignedWorkerId },
            ]);
  ...
  const handleOrderAction = async (order) => {
    ...
    try {
      ...
      const isInWorkingTab = workingOrders.some(
      ...
      if (isInWorkingTab) {
        ...
        switch (userRole) {
          ...
          case "receiving_clerk":
            if (currentStatus === "processing") {
              result = await workflowOrderService.processToPickingOrder(
                orderId
              );
            }
            break;
          case "Picker":
            if (currentStatus === "picking") {
              result = await workflowOrderService.completePickingOrder(
                orderId,
                workerId
              );
            }
          ...
          case "Packer":
            if (currentStatus === "packing") {
              result = await workflowOrderService.completePackingOrder(
                orderId,
                workerId
              );
            }
        ...
        if (result?.success) {
          const actionLabel = workflowOrderService.getRoleActionLabel(
            currentStatus,
            userRole
          );
        ...
        if (userRole === "Manager" && currentStatus === "pending") {
          ...
          result = await workflowOrderService.assignOrderToWorker(
            orderId,
            workerId
          );
  ...
  const handleOptimalPath = async (order) => {
    try {
      const result = await workflowOrderService.getOptimalPickingPath(
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div
                className={`flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br ${gradientColors} text-white shadow-lg`}
              >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {currentTabOrders.map((order) => (
                  <OrderCard
                    ...
                    isProcessing={
                      processingOrderId === (order.order_id || order.orderID)
                    }
                    showOptimalPath={
                      userRole === "Picker" && activeTab === "working"
                    }
