...
const useOrderWebSocket = () => {
  ...
  const getAuthData = useCallback(() => {
    ...
    console.log('🔑 Auth data:', { token: token ? 'present' : 'missing', isAuth });
    return { token, isAuth };
  }, [isAuthenticated]);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (event.code !== 1000 && reconnectAttempts.current < maxReconnectAttempts && currentToken && currentIsAuth) {
          reconnectAttempts.current++;
          console.log(`🔄 Auto-reconnect attempt ${reconnectAttempts.current}/${maxReconnectAttempts} in ${reconnectDelay}ms`);
  ...
  useEffect(() => {
    ...
    console.log('🔍 Current auth state:', { token: token ? 'present' : 'missing', isAuth });
    
    if (token && isAuth) {
      console.log('🔄 Token available, attempting WebSocket connection');
      connect();
    } else {
      console.log('⏸️ No token or not authenticated, skipping WebSocket connection');
      disconnect();
    }
